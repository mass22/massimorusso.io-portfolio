  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        const errorMessage = errorData?.message || 'Domaine non vérifié'
        ...
        console.error('[Email]   1. Pour les tests: Utilisez votre email Resend (russo.mass@gmail.com) comme ADMIN_EMAIL')
